...
type RoomSettingsDraft = { max_players: string; round_count: string; selection_duration: string; question_duration: string; between_question_duration: string };
...
function ParentGameWait({ phase, parentName }: { phase: "selecting" | "answering"; parentName: string }) {
  ...
  return <div className={`parent-game-wait wait-${phase}`} role="status">
    ...
    <strong>{answering ? "親が回答を選んでいます" : "親が問題を選び中です"}</strong>
    <p>{answering ? `${parentName}さんの選択は、結果発表まで秘密です` : `${parentName}さんが今回の問題を選んでいます…`}</p>
  </div>;
}
...
export default function RoomPage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  const [roomSettingsDraft, setRoomSettingsDraft] = useState<RoomSettingsDraft>({ max_players: "12", round_count: "1", selection_duration: "15", question_duration: "20", between_question_duration: "5" });
  ...
  return <main id="main-content" className="game-page">
    ...
    {state.status === "SELECTING" && <section className="card parent-selection-card" aria-labelledby="parent-selection-title">
    ...
    {state.status === "PARENT_ANSWERING" && !isCurrentParent && <section className="card parent-selection-card parent-answer-wait" aria-labelledby="parent-answer-wait-title"><span className="step-label">親が先に選択します</span><h2 id="parent-answer-wait-title" className="visually-hidden">親の回答待ち</h2><ParentGameWait phase="answering" parentName={currentParent?.username || "親"} /></section>}
    ...
    {(state.status === "QUESTION" || (state.status === "PARENT_ANSWERING" && isCurrentParent)) && state.question && <section className={`card question-card${state.status === "PARENT_ANSWERING" ? " parent-first-answer" : ""}`} aria-labelledby="question-title">
      <div className="question-meta"><span>ラウンド {state.current_round} / {state.round_count} · {state.status === "PARENT_ANSWERING" ? "親の先行回答" : `親：${currentParent?.username || "—"}`}</span>{state.status === "QUESTION" ? <strong className="timer" role="timer">{remaining}<small>秒</small></strong> : <strong className="parent-secret-badge">SECRET</strong>}</div>
      {state.status === "QUESTION" && <div className="time-track" aria-hidden="true"><i style={{ width: `${phaseProgress}%` }} /></div>}
