...
export default function AdminDashboard({ section }: { section: Section }) {
  ...
  const [deleteAllOpen, setDeleteAllOpen] = useState(false); const [deletingAll, setDeletingAll] = useState(false);
  ...
  async function deleteAllRooms() {
    if (rooms.length === 0 || deletingAll) return;
    setDeletingAll(true);
    try {
      const result: { deleted: number } = await request("/api/admin/rooms", { method: "DELETE" });
      setRooms([]); setSelectedRoom(null); setDeleteAllOpen(false);
      setMessage(`${result.deleted}件のルームを削除しました。`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "全ルームの削除に失敗しました。"); }
  ...
  return <main id="main-content" className="admin-page"><header className="admin-header"><div><span className="eyebrow">ゲーム管理</span><h1>管理コンソール</h1><p className="muted">ルーム、質問、ゲーム進行を管理します。</p></div><div className="button-row"><button type="button" className="secondary" onClick={logout}>ログアウト</button><a className="secondary admin-button" href="/">ロビーに戻る</a></div></header><nav className="admin-nav" aria-label="管理画面ナビゲーション">{nav.map(item => <Link key={item.section} href={item.href} className={section === item.section ? "active" : ""} aria-current={section === item.section ? "page" : undefined}>{item.label}</Link>)}</nav>{message && !(section === "questions" && questionEditorOpen) && <p className="notice" role="status" aria-live="polite">{message}</p>}
    ...
    {section === "rooms" && <>
      <section className="card admin-room-list-card">
        <div className="row admin-room-toolbar"><div><span className="step-label">ルーム管理</span><h2>ルーム一覧</h2><p className="muted">{rooms.length}件 · 1.5秒ごとに自動更新</p></div><div className="button-row"><button type="button" className="danger delete-all-rooms-button" disabled={rooms.length === 0} onClick={() => setDeleteAllOpen(true)}>全ルームを削除</button><button type="button" className="secondary" onClick={() => void loadRooms()}>更新</button><button type="button" onClick={createRoom}>ルームを作る</button></div></div>
        {rooms.length === 0 ? <div className="empty admin-room-empty"><span className="empty-mark" aria-hidden="true">0</span><div><h3>ルームはありません</h3><p className="muted">新しいルームを作ると、ここに表示されます。</p></div></div> : <div className="room-list admin-room-list">{rooms.map(room => <article className={`room-card admin-room ${selectedRoom?.room_id === room.room_id ? "selected" : ""}`} key={room.room_id}><span className={`status status-${room.status.toLowerCase()}`}>{statusLabel(room.status)}</span><div className="admin-room-copy"><h3>{room.settings.game_name}</h3><code>{room.room_id}</code></div><div className="admin-room-counts" aria-label={`${room.players.length}/${room.settings.max_players}人、準備完了${room.players.filter(player => player.ready).length}人`}><span><strong>{room.players.length}</strong>/{room.settings.max_players}人</span><span>準備 <strong>{room.players.filter(player => player.ready).length}</strong></span></div><button type="button" className="secondary" onClick={() => chooseRoom(room)}>管理</button></article>)}</div>}
      </section>
      <BottomSheet open={deleteAllOpen} onClose={() => { if (!deletingAll) setDeleteAllOpen(false); }} labelledBy="delete-all-rooms-title" describedBy="delete-all-rooms-summary" closeLabel="キャンセル" className="delete-all-rooms-sheet" header={<><span className="step-label danger-label">取り消せない操作</span><h2 id="delete-all-rooms-title">全ルームを削除しますか？</h2><p id="delete-all-rooms-summary" className="muted">現在の{rooms.length}件をすべて削除し、参加中のプレイヤーも退出します。</p></>}>
        <div className="delete-all-room-warning"><strong>削除後は元に戻せません。</strong><p>進行中のゲーム、スコア、回答履歴も失われます。</p></div>
        <div className="button-row delete-all-room-actions"><button type="button" className="secondary" disabled={deletingAll} onClick={() => setDeleteAllOpen(false)}>キャンセル</button><button type="button" className="danger" disabled={deletingAll || rooms.length === 0} onClick={() => void deleteAllRooms()}>{deletingAll ? "削除しています…" : `${rooms.length}件をすべて削除`}</button></div>
      </BottomSheet>
      {selectedRoom && <BottomSheet open onClose={() => setSelectedRoom(null)} labelledBy="room-sheet-title" describedBy="room-sheet-summary" className="admin-room-sheet" header={<><span className={`status status-${selectedRoom.status.toLowerCase()}`}>{statusLabel(selectedRoom.status)}</span><h2 id="room-sheet-title">ルーム {selectedRoom.room_id}</h2><p id="room-sheet-summary" className="muted">{selectedRoom.status === "WAITING" ? `${selectedRoom.round_count}ラウンド` : `ラウンド ${selectedRoom.current_round} / ${selectedRoom.round_count} · 第 ${Math.min(selectedRoom.current_question_index + 1, selectedRoom.question_count)} / ${selectedRoom.question_count} ターン`}</p></>}>
            ...
            {selectedRoom.question && <div className="inset-form"><strong><QuestionText title={selectedRoom.question.title} /></strong><p className="muted">押す ／ 押さない</p>{selectedRoom.result && <p>押す {selectedRoom.result.counts.A} · 押さない {selectedRoom.result.counts.B}</p>}</div>}
            {selectedRoom.status === "SELECTING" && <div className="inset-form"><strong>{selectedRoom.players.find(player => player.id === selectedRoom.current_parent_id)?.username || "親"}</strong><p className="muted">今回の問題を選んでいます。</p></div>}
    ...
    {section === "questions" && <>
      <section className="card question-library">
        ...
        {questions.length === 0 ? <div className="empty"><span className="empty-mark" aria-hidden="true">？</span><div><h3>質問はまだありません</h3><p className="muted">最初の質問を追加してゲームを始めましょう。</p></div></div> : <div className="question-list">{questions.map(question => <article className="question-row" key={question.id}>
          ...
          <div className="question-row-copy"><h3><QuestionText title={question.title} /></h3><div className="question-options"><span><b>●</b>押す</span><span><b>—</b>押さない</span></div></div>
      ...
      <BottomSheet open={questionEditorOpen} onClose={closeQuestionEditor} labelledBy="question-editor-title" describedBy="question-editor-summary" closeLabel="キャンセル" className="question-editor-sheet" header={<><span className="step-label">{editingQuestion ? `質問 ${String(editingQuestion.order).padStart(2, "0")}` : "新しい質問"}</span><h2 id="question-editor-title">{editingQuestion ? "質問を編集" : "質問を作成"}</h2><p id="question-editor-summary" className="muted">ボタンを押すと起きることを入力します。</p></>}>
        <form className="editor-form question-editor-form" onSubmit={saveQuestion}>
          <label htmlFor="question-title">ボタンで起きること<input id="question-title" value={draft.title} onChange={event => setDraft({ ...draft, title: event.target.value })} placeholder="例：毎朝好きな時間まで眠れる。しかし、毎晩必ず怖い夢を見る。" pattern=".*しかし.*" title="転換語「しかし」を必ず入れてください。" required /></label>
          {draft.title && <div className="question-preview" aria-label="質問のプレビュー"><span>プレビュー</span><strong><QuestionText title={draft.title} /></strong></div>}
          <p className="notice">選択肢は「押す」と「押さない」で固定されるため、入力は不要です。</p>
